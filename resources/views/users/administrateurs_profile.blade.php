...
@section('page_title', $admin->nom . ' ' . $admin->prenoms)
    ...
    <div class="col-md-3">
      <div class="card card-primary card-outline">
        <div class="card-body box-profile">
          ...
          <form id="avatarForm" action="{{ route('users.administrateurs.update', $admin) }}" method="POST" enctype="multipart/form-data" class="mt-2">
            @csrf
            @method('PUT')
            <input type="hidden" name="redirect_to" value="{{ route('users.administrateurs.show', $admin) }}">
            <input type="file" name="avatar" id="adminAvatarInput" accept="image/*" class="d-none">
          ...
          <h3 class="profile-username text-center">{{ $admin->nom }} {{ $admin->prenoms }}</h3>
          <p class="text-muted text-center">{{ $admin->contact }}</p>

          <ul class="list-group list-group-unbordered mb-3">
            <li class="list-group-item">
              <b>Login</b>
              <a class="float-right">{{ $admin->login }}</a>
            </li>
            <li class="list-group-item">
              <b>Statut</b>
              <a class="float-right">{{ $admin->statut_compte ? 'Actif' : 'Inactif' }}</a>
            </li>
          </ul>

          <a href="{{ route('users.administrateurs') }}" class="btn btn-primary btn-block"><b>Retour</b></a>
        </div>
      </div>
    </div>
        ...
        <div class="card-body">
          <div class="tab-content">
            <div class="tab-pane" id="activity">
              <form action="{{ route('users.administrateurs.update', $admin) }}" method="POST">
                @csrf
                @method('PUT')
                <input type="hidden" name="redirect_to" value="{{ route('users.administrateurs.show', $admin) }}">
                ...
                <div class="form-group">
                  ...
                  <input type="text" name="prenoms" id="prenoms" class="form-control" value="{{ $admin->prenoms }}" required>
                </div>
            ...
            <div class="active tab-pane" id="settings">
              <form action="{{ route('users.administrateurs.update', $admin) }}" method="POST">
                ...
                <div class="form-group">
                  ...
                  <input type="text" name="contact" id="contact" class="form-control" value="{{ $admin->contact }}" required>
                </div>

                <div class="form-group">
                  <label for="login">Login</label>
                  <input type="text" name="login" id="login" class="form-control" value="{{ $admin->login }}" required>
                </div>

                <div class="form-group">
                  <label for="statut_compte">Statut</label>
                  <select name="statut_compte" id="statut_compte" class="form-control">
                    <option value="1" {{ $admin->statut_compte ? 'selected' : '' }}>Actif</option>
                    <option value="0" {{ !$admin->statut_compte ? 'selected' : '' }}>Inactif</option>
                  </select>
                </div>
