...
<head>
  ...
  <title>OVL Admin | @yield('title', 'Dashboard')</title>
...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav">
      ...
      <li class="nav-item d-none d-sm-inline-block">
        <a href="{{ route('bilans.index') }}" class="nav-link">Bilans</a>
      </li>
      <li class="nav-item d-none d-sm-inline-block">
        <a href="{{ route('dashboard') }}" class="nav-link">Analytics</a>
      </li>
    </ul>
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item">
        <form method="POST" action="{{ route('logout') }}" class="d-inline">
          @csrf
          <a href="#" class="nav-link" role="button" title="Déconnexion" onclick="event.preventDefault(); this.closest('form').submit();">
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <a href="index3.html" class="brand-link">
      ...
      <span class="brand-text font-weight-light">OVL Admin</span>
    </a>
    ...
    <div class="sidebar">
      ...
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        <div class="image">
          @php
            $u = \Illuminate\Support\Facades\Session::get('utilisateur', []);
            if (!is_array($u)) {
              $u = [];
            }
            ...
            $currentUser = null;
            if (!empty($u['id'] ?? null)) {
              $currentUser = \App\Models\Utilisateur::query()->find($u['id']);
            }

            $defaultSidebarAvatarUrl = asset('dist/img/user2-160x160.jpg');
            $sidebarAvatarUrl = $defaultSidebarAvatarUrl;

            $avatarKey = $currentUser?->avatar ?? ($u['avatar'] ?? null);
            $role = $currentUser?->role ?? ($u['role'] ?? null);

            if (!$avatarKey || $avatarKey === 'default.jpg') {
              $avatarKey = $role === 'admin' ? 'administrateurs/admins.png' : 'utilisateurs/utilisateurs.png';
            } elseif (!str_contains($avatarKey, '/')) {
              $avatarKey = $role === 'admin' ? 'administrateurs/' . $avatarKey : 'utilisateurs/' . $avatarKey;
            }
            ...
            try {
              ...
              try {
                ...
                $sidebarAvatarUrl = $disk->url($avatarKey);
              }
            } catch (\Throwable $e) {
              $sidebarAvatarUrl = $defaultSidebarAvatarUrl;
            }

            $sidebarUserName = trim(($currentUser?->nom ?? ($u['nom'] ?? '')) . ' ' . ($currentUser?->prenoms ?? ($u['prenoms'] ?? '')));
            if ($sidebarUserName === '') {
              $sidebarUserName = $currentUser?->login ?? ($u['login'] ?? 'Utilisateur');
            }
          @endphp

          <img src="{{ $sidebarAvatarUrl }}" class="img-circle elevation-2" alt="" style="width: 34px; height: 34px; object-fit: cover;" onerror="this.onerror=null;this.src='{{ $defaultSidebarAvatarUrl }}';">
        </div>
        <div class="info">
          <a href="#" class="d-block">{{ $sidebarUserName }}</a>
        </div>
      </div>
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="{{ route('bilans.index') }}" class="nav-link">
                  <i class="fas fa-chart-line nav-icon"></i>
                  <p>Point du jour</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="{{ route('bilans.hier') }}" class="nav-link">
          ...
          <li class="nav-item">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="{{ route('points-livreurs.index') }}" class="nav-link">
          ...
          <li class="nav-item">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="{{ route('factures.payees') }}" class="nav-link">
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{ route('bilans.index') }}">Accueil</a></li>
